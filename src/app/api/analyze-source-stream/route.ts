...
- personName: 人名のみ（姓名のみ。例: 田中太郎、豊田章男）。記事タイトル・見出し・役職名・企業名は絶対に入れない。見つからない場合は空文字。
...
例1: トヨタ自動車 (toyota.co.jp)
- [数値ファクト] 2024年3月期の連結売上高は45兆953億円（前年比21.4%増）
- [経営課題] 「マルチパスウェイ戦略」でBEV・HEV・FCEV・水素エンジンの全方位開発を推進
- [Why you now] 2024年1月のダイハツ不正問題を受け、グループガバナンス強化が最重要課題に
- ✗「トヨタは日本最大の自動車メーカーです」→ 一般常識は不要
- ✗「DXに取り組んでいます」→ 全企業に当てはまる一般論は不可

例2: サイバーエージェント (cyberagent.co.jp)
- [数値ファクト] 2024年9月期のAbema事業は売上高約700億円、初の通期黒字を達成
- [経営課題] 生成AI活用による広告クリエイティブ自動生成で広告事業の利益率改善を推進
- [Why you now] Abema黒字化でメディア事業の投資フェーズから収益化フェーズに転換
- ✗「インターネット広告大手です」→ 固有の情報がない
- ✗「グローバル展開しています」→ 具体性ゼロ

例3: デジタル庁 (digital.go.jp)
- [数値ファクト] マイナンバーカード交付枚数が累計1億枚を突破（2024年3月時点）
- [経営課題] 自治体システムの標準化・共通化を2025年度末までに完了する期限が迫る
- [Why you now] ガバメントクラウド移行の第2期公募が開始、民間連携ニーズが拡大
- ✗「行政のデジタル化を推進しています」→ 組織の説明は不要
- ✗「国民の利便性向上を目指しています」→ 当然すぎて差別化不可
...
- 上記の良い分析例のように、数値・固有名詞・Why you nowを含む具体的なファクトを抽出すること
- 「〜大手です」「〜に取り組んでいます」のような一般常識・一般論は抽出しないこと
