...
- プレーンテキストのみで出力してください
- **プレースホルダー完全禁止**: 「〇〇」「●●」「xx」「△△」などの伏せ字は絶対に使用しないこと。具体的な数値がない場合は「大幅な改善」「多くの企業様」などの定性的表現に言い換えるか、その文言を削除すること。
- **姓名順序**: 宛名の氏名は必ず「姓 名」の順で記載すること（例: 山田 太郎 様）。「名 姓」の順は絶対に使用しないこと。
- **具体性**: 提供された企業名やサービス名を必ず使用し、一般論ではなく相手企業に刺さる内容にすること。
- **論理構成**: 相手企業を適切に称えつつ、自社の提案が自然な解決策に見えるよう構成すること。
`;
