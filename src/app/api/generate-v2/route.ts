...
- OK: 「製造業の経営企画・内部監査の責任者クラス」
- OK: 「大手製造業の実務責任者」

■ ルールe: セミナー価値は"持ち帰り"を最低3点
具体的なベネフィットを3つ以上明記:
- チェックリスト（例: 〇〇対応の自己診断シート）
- 標準プロセスの型（例: 〇〇導入のフレームワーク）
- 監査観点整理（例: 指摘されやすいポイント一覧）

■ ルールf: CTAは2択で1クリック設計
返信を簡単にする:
- OK: 「ご参加希望の場合は『参加』とご返信ください」
- OK: 「ご都合が合わない場合は『資料希望』とご返信ください」
- NG: 「ぜひ一度15分お時間をいただき」← 面談要求禁止
- NG: 「資料送付と合わせてお打ち合わせの機会を」← 二重CTA禁止

■ ルールg: 文章は短め（5段落以内、300-450文字）
- 全体を5段落以内に収める
- 各段落は2-3文を目安
...
【品質強化ルール（CxOレター必須・違反時リライト）】

■ 宛名ルール
- 個人名がある場合: 「〇〇株式会社　田中様」
- 個人名がない場合: 「〇〇株式会社　ご担当者様」
- NG: 「ご担当者 田中様」（二重表現）

■ 推察・想定・断定診断の完全禁止【HARD NO】
以下の表現は絶対に使用禁止（違反時は即リライト）:
- NG: 「推察します」「推察いたします」「と推察」
- NG: 「想定されます」「と想定」
- NG: 「統一されておらず」「残存している」「残存し」
- NG: 「課題が生じている」「〇〇ではないでしょうか」（相手の状況を決めつける場合）
- NG: 「感銘を受けました」（儀礼的）

代わりに「一般論型」＋「確認型」を使用:
- OK: 「多くのグループ企業では、〇〇が論点になりやすいです」
- OK: 「一般に〇〇が課題となりやすいと言われています」
- OK: 「御社における現状の論点と優先度だけ伺い」

■ テーマ接続ルール（ワープ禁止）
フック（導入）と提案テーマは必ず1行の橋渡しで接続する:
- NG: 「サステナ更新を拝見→内部統制が重要」（ワープ）
- OK: 「サステナ更新を拝見→サステナ推進ではデータ基盤と統制が前提→経費/出張データはサステナ開示のデータ基盤にも効く→内部統制」

橋渡し文の例:
- 「〇〇推進では、グループ全体で一貫したデータの収集と統制が効いていることが前提になりやすいと感じております」
- 「〇〇領域では、△△の仕組みも提供しており、□□とも接続しやすい設計です」

■ ベネフィット絞り込み（1本＋補助線）
CxOの頭で重要度が並列になるのを避ける:
- メイン1本: 監査/統制/コンプライアンス等から1つ選ぶ
- 補助線1本: サステナ/可視化等で補強（1文のみ）
- NG: 「標準化」「工数削減」「可視化」「迅速化」が全部並ぶ

■ プレースホルダー完全禁止（完成モード）【HARD NO】
以下は完成モードでは絶対禁止:
- 「〇〇」「●●」「△△」「□□」
- 「〇週間」「〇名」「〇社」
- 「【要確認: 〇〇】」
...
数値が不明な場合の対処:
- NG: 「〇〇名規模/複数社グループでの運用を前提に設計」
- OK: 「複数社グループでの運用を前提に設計」（数字を消す）
- OK: 「段階導入が可能です」（期間を消す）

■ 差別化・証拠（90点化の鍵）
数値なしでも以下は書ける:
- 「証憑紐づけと承認ログの証跡を標準で揃えます」
- 「段階導入が可能です」
- 「既存業務を大きく変えずに開始できます」
`;
