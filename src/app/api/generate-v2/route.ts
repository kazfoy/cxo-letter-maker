...
■ 無根拠診断の禁止（CxOの防御反応を避ける）
相手企業の内部事情を決めつける表現は使用禁止:
- NG: 「貴社では〇〇が統一されておらず」
- NG: 「紙ベースの管理が残存し」
- NG: 「〇〇に課題が生じているといった状況も想定されます」
- NG: 「貴社では〇〇が課題と推察いたします」

代わりに「一般論型」＋「確認型」を使用:
- OK: 「一般にグループ規模が大きいほど、〇〇が論点になりやすいです」
- OK: 「御社における現状の論点と優先度だけ伺い」
- OK: 「業界各社で〇〇への対応が進む中」

■ フックと提案テーマの接続強化（ブリッジ構造）
冒頭200文字以内に以下の流れを必ず含める:
1. フック: 相手固有の情報を引用（「〇年〇月の〜を拝見し」）
2. ブリッジ文: フックと提案テーマをつなぐ1文
   - 「攻めの施策が増えるほど、統制が効いていることが前提になる」
   - 「このような変化の中で、〜がより重要になる」
3. 仮説: 柔らかく提示（断定しない）

■ 価値文の重複禁止（コンパクトに）
同じ価値を複数回説明しない:
- NG: 「一元化→標準化」「標準化→ガバナンス強化」「見える化→意思決定スピード」（3回重複）
- OK: 1つの価値を1回だけ、具体的に説明（2文以内）

全体の文字数: 300-450文字（5段落以内）

■ 差別化・証拠の必須化（90点化の鍵）
以下のいずれかを必ず1つ以上含める:
- 導入期間の目安: 「最短◯週間で、特定領域から段階導入が可能です」
- 連携方針: 「ERP/会計との連携は◯◯方式で、既存業務を大きく変えずに開始できます」
- 監査アウトプット: 「証憑紐づけと承認ログの証跡を標準で揃えます」
- 規模感: 「◯◯名規模/複数社グループでの運用を前提に設計しています」
