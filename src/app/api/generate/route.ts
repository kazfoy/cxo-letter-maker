...
【品質ルール（厳守）】
* **プレースホルダー完全禁止**:
  * 「〇〇」「●●」「xx」「△△」「□□」などの伏せ字・プレースホルダーは絶対に使用しないこと。
  * 具体的な数値やデータがない場合は、「大幅な改善」「多くの企業様」「業界トップクラス」などの定性的表現に言い換えるか、その文言自体を削除すること。
* **具体性の確保**:
  * 提供された企業名、サービス名、役職名などの情報は必ずそのまま使用すること。
  * 相手企業の業界特性や事業内容に言及し、一般論ではなく相手に刺さる内容にすること。
  * 「御社」「貴社」だけでなく、可能な限り具体的な企業名を使用すること。
* **宛名の姓名順序**:
  * 日本語の手紙では必ず「姓 名」の順で記載すること（例: 山田 太郎 様）。
  * 「名 姓」の順（例: 太郎 山田）は絶対に使用しないこと。
* **論理構成**:
  * 相手企業を適切に称えつつ、自社の提案が自然な解決策に見えるよう論理的に構成すること。
  * 唐突な売り込みではなく、相手の課題理解→解決策の流れを意識すること。
`;
